...
module.exports = function unauthorized(data, options) {
  ...
  return res.view('401', { data: viewData, title: 'Unauthorized' }, (err, html) => {
    ...
    if (err) {
      ...
      if (err.code === 'E_VIEW_FAILED') {
        sails.log.verbose('res.unauthorized()' +
          ':: Could not locate view for error page (sending JSON instead).  Details: ', err);
        // Otherwise, if this was a more serious error, log to the console with the details.
      } else {
        sails.log.warn('res.unauthorized()' +
          ':: When attempting to render error page view, an error occured (sending JSON instead).  Details: ', err);
      }
      return res.jsonx(data);
    }
